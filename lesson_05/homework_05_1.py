...
for brand, (color, year, engine, type, price) in car_data.items():
    if year >= search_criteria[0]  and engine >= search_criteria[1] and price <= search_criteria[2]:
        matching_cars.append((brand, color, year, engine, type, price))

matching_cars.sort(key=lambda x: x[5]) ## вот тут возникли трудности, пришлось гуглить)
if matching_cars:
    print("Search results:")
    for car in matching_cars[:5]:
        brand, color, year, engine, type_, price = car
        print(f"{brand}: {year}, {engine}, {type}, {color}, {price}$")
else:
    print("No data found.")
